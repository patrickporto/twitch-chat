...
export function parseMessage(message: string) {
  let parsedMessage: TwitchMessage = {
  ...
  if (null == parsedMessage.command || !rawSourceComponent) {
...
function parseTags(tags: string) {
  ...
  let dictParsedTags: Record<string, any> = {}; // Holds the parsed list of tags.
  ...
  parsedTags.forEach((tag) => {
    ...
    switch (
      ...
      case "badge-info":
        ...
        if (tagValue) {
          let dict: Record<string, any> = {}; // Holds the list of badge objects.
      ...
      case "emotes":
        ...
        if (tagValue) {
          let dictEmotes: Record<string, any> = {}; // Holds a list of emote objects.
...
function parseParameters(rawParametersComponent: string, command: Command) {
  let idx = 0;
  ...
  if (-1 == paramsIdx) {
    // no parameters
    command.botCommand = commandParts.slice(0) as TwitchCommand;
  } else {
    command.botCommand = commandParts.slice(0, paramsIdx) as TwitchCommand;
